...
namespace Web.Infrastructure.Service.File;
...
public sealed class ExcelDateFormatAttribute : Attribute
{
    ...
    private void Fill(ExcelWorksheet sheet, List<string> data)
        {
            for (int i = 0; i < data.Count; i++)
                sheet.Cells[i + 1, 1].Value = data[i];
        }

        private void AddDropdown(ExcelWorksheet sheet, int colIndex, string formula)
        {
            var colLetter = GetColumnName(colIndex);

            var validation = sheet.DataValidations.AddListValidation($"{colLetter}2:{colLetter}1000");
